...
//
// For example, if the qubits passed to your operation are in the state 1/√2(|00⟩+|01⟩)=1/√2(|0⟩+|2⟩), they should
// be transformed to the state 1/√2(|(0−1)mod22⟩+|(2−1)mod22⟩)=1/√2(|3⟩+|1⟩)=1/√2(|11⟩+|10⟩).
...
operation Main() : Unit
{
    ...
    // Prepare the input state 1/√2(|00⟩ + |01⟩)
    H(qs[1]);
    
    DumpRegister(qs);

    // Call the Adjoint Solve operation from B1 to decrement the number in the register modulo 2^N.
    Adjoint Solve(qs);
